...
//            187
    ...
    public static void main(String[] args) throws IOException {
        ...
        System.out.println(new AlphaMath1339().run(br, lines));
        br.close();
    }
    ...
    public int run(BufferedReader br, int lines) throws IOException {
        ...
        for(int j = 0 ; j < lines ; j ++){
            ...
            int size = givenArr.length;
            for(int i = size-1 ; i >= 0 ; i --){
                map.put(givenArr[i], map.getOrDefault(givenArr[i], 0.0) + Math.pow(10, i));
            }
        }

        List<Character> keySetList = new LinkedList<Character>(map.keySet());
        Collections.sort(keySetList, Comparator.comparingDouble(map::get).reversed());

        int index = 9;

        for(Character k : keySetList){
            result += index * map.get(k);
            index --;
        }
